...
FROM (SELECT constituency, party, votes, rank() OVER (PARTITION BY constituency order by votes desc) rank
      FROM ge
      WHERE constituency BETWEEN 'S14000021' AND 'S14000026'
      AND yr  = 2017) as sub
WHERE rank = 1
...
SELECT party, COUNT(*)
FROM (SELECT constituency, party, votes, rank() OVER (PARTITION BY constituency order by votes desc) as rank
      FROM ge
